...
//SizedBox(height: 35.sp,),
                    ...
                    Container(margin: EdgeInsets.symmetric(horizontal:1.sw/7.5),
                      color: AppColors.primaryColor,

                      child: Center(child: Text("    MY\nLIBRARY",style: GoogleFonts.lexend(fontSize: 50.sp,fontWeight: FontWeight.w700),)),
                    ),
  ...
  children: <Widget>[
      ...
      Expanded(
        ...
        child:  TouchableOpacity(
          onTap: (){
            if(pageIndex!=0) {
              AppControllers.libraryPageController.jumpToPage(0);
            context.read<LibraryPageViewCubit>().setPageIndex(0);
          ...
          child: Container(
            height: 35.sp,
            width: 80.sp,
            margin: EdgeInsets.only(top: 2.sp,bottom: 2.sp),
              decoration: BoxDecoration(
                color: pageIndex==0?AppColors.libraryMainColor:Colors.transparent,
                borderRadius: BorderRadius.circular(20.sp)
                  ,
                border: Border.all(color:pageIndex==0? Colors.black:Colors.transparent,width: 2.0)
              ),
              child: Center(child: Text("BOOKS",style: GoogleFonts.lexend(fontWeight: FontWeight.bold,fontSize: 15.sp),))),
        ),
      )),
      Spacer(),
      Expanded(
...
if(pageIndex!=1) {
  ...
  context.read<LibraryPageViewCubit>().setPageIndex(1);
            ...
            margin: EdgeInsets.only(top: 2.sp,bottom: 2.sp),
            decoration: BoxDecoration(
                color: pageIndex==1?AppColors.libraryMainColor:Colors.transparent,
                borderRadius: BorderRadius.circular(20.sp)
                ,
                border: Border.all(color: pageIndex==1? Colors.black:Colors.transparent,width: 2.0)
            ),
            child: Center(child: FittedBox(child: Text("DISCOVER",style: GoogleFonts.lexend(fontWeight: FontWeight.bold,fontSize: 15.sp))))),
          ))),
      Spacer(),
...
Expanded(
  child:   PageView(
      ...
      controller: AppControllers.libraryPageController,
      onPageChanged: (x){

        AppControllers.libraryPageController.jumpToPage(x);
        context.read<LibraryPageViewCubit>().setPageIndex(x);
      },
      children: [
        BooksLibrary(),
        DiscoveryTab()
